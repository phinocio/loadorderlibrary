...
-   Added an "Orphaned Files" section to admin stats to show me files that are no longer in a list. The idea is it _should_ be empty
...
-   Fixed `show disabled` function only toggling the _first_ disabled item in modlist.txt. Toggling now works as intended
